        ...
        if request.method == "OPTIONS":
            ...
            response = Response(
                ...
                headers={
                    ...
                    "Access-Control-Allow-Headers": "Content-Type, Authorization, X-Requested-With, Cookie, X-Session-ID",
                    "Access-Control-Allow-Credentials": "true",
                    "Access-Control-Expose-Headers": "X-Session-ID",  # 允许前端读取响应头中的 session_id
...
app.add_middleware(
    ...
    allow_origins=[
        ...
        "https://musicmove.pawazzz.xyz",  # 添加自定义域名
        "http://localhost:3000",
        "https://music-move.onrender.com",
    ],
    allow_credentials=True,  # 允许跨域请求携带凭据（Cookie）
    allow_methods=["*"],
    allow_headers=["*", "X-Session-ID"],  # 明确允许 X-Session-ID 请求头
    expose_headers=["*", "X-Session-ID"],  # 允许前端读取 X-Session-ID 响应头
